...
/*   Updated: 2019/10/29 19:26:10 by kbatz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	avl_int_bfs(t_avl_int *root, void (*f)(t_avl_int *node))
{
	...
	while (queue->len)
	{
		...
		if (root->left)
			ft_queue_push(queue, ft_new_sq_elem(root->left, sizeof(t_avl_int), 0));
		if (root->right)
			ft_queue_push(queue, ft_new_sq_elem(root->right, sizeof(t_avl_int), 0));
		(*f)(root);
		free(tmp);
	}
}
